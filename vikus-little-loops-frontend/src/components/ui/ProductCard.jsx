...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="rounded-xl2 border border-blush-300 shadow-soft transition-shadow duration-500 hover:shadow-lift">
      <motion.div
        variants={fadeUp}
        whileHover={{ y: -12 }}
        transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
        className="group relative overflow-hidden rounded-xl2 bg-ivory"
      >
        <Link to={`/product/${p.slug}`} className="block">
          {/* Image frame — strictly 4:3 */}
          <div
